        ...
        if (process.platform === 'win32') {
            ...
            setTimeout(() => {
                if (null === CF.Window.nextWindow) {
                    ...
                    nextWindow.loadURL(`${CF.debug ? 'http://localhost:9080/' : `file://${__dirname}/index.html`  /*file://${__dirname}/index.html*/}`);
    ...
    _draw(show, args, callback) {
        ...
        if (hasWindow) {
            ...
            let cb = async () => {
                ...
                let script = async function(url, args, debug, key, data) {
                    window.onLoadHtml(url, args);
                };
            ...
            win.once('ready-to-show', () => {
                ...
                let script = async function(url, args, debug, key, data) {
                    window.onLoadHtml(null, args);
                };
                win.webContents.executeJavaScript(';(' + script.toString() + ')(' + JSON.stringify(this.url) + ', ' + JSON.stringify(args) + ', ' + CF.debug + ');', true);
